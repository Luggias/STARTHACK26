...
> **Role:** This document defines the operational constraints and coding standards for AI Agents and automated contributors. Adherence is mandatory to ensure project stability and maintainability.
...
* **No Hardcoding:** Content values must be passed via props, state, or schemas. Never embed raw strings inside UI components.
...
* **Predictable Data Flow:** Data flows uni-directionally. Minimize side effects. In an HFT context, state transitions must be explicit and traceable.
...
2.  **Plan:** State the intended approach in English. If the change impacts the "hot path" or core architecture, wait for explicit user confirmation.
